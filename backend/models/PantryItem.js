...
class PantryItem {
    ...
    static async create(userId, itemData) {
        const { name, category, quantity, unit, expiry_date, is_running_low = false } = itemData;
        const result = await db.query(
            'INSERT INTO pantry_items (user_id, name, category, quantity, unit, expiry_date, is_running_low) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
            [userId, name, category, quantity, unit, expiry_date, is_running_low]
        );
    ...
    static async update(id, userId, updates) {
        const { name, category, quantity, unit, expiry_date, is_running_low } = updates;
        const result = await db.query(
            `UPDATE pantry_items 
            SET name = COALESCE($1, name), 
                ...
                RETURNING *`,
            [name, category, quantity, unit, expiry_date, is_running_low, id, userId]
        );
        return result.rows[0];
    }
