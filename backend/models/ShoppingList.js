...
class ShoppingList {
    /**
     * Genera shopping list from meal plan
     */
    static async generateFromMealPlan(userId, startDate, endDate) {
        ...
        try {
            ...
            // Get all ingredients frm meal plan recipes
            const result = await client.query(
                `SELECT ri.ingredients_name, ri.unit, SUM(ri.quantity) as total_quantity
                FROM meal_plans mp
                JOIN recipe_ingredients ri ON mp.recipe_id
                WHERE mp.user_id = $1
                AND mp.meal_date >= $2
                AND mp.meal_date <= $3
            ...
            // Insert shopping list items (substract pantry quantities)
            for(const ing of ingredients) {
                ...
                if (neededQty >0) {
                    await client.query(
                        ...
                        (userId, ingredient_name, quantity, unit, from_meal_plan, category)
    ...
    static async create(userId, itemData) {
        const { ingredients_name, quantity, unit, category = "Uncategorized" } = itemData;

        const result = await db.query(
            `INSERT INTO shopping_list_items
            (userId, ingredient_name, quantity, unit, category, from_meal_plan)
            VALUES ($1, $2, $3, $4, $5, false)
            RETURNING *`,
            [userId, ingredients_name, quantity, unit, category]
        );
    ...
    static async addCheckedToPantry(userId) {
        ...
        try {
            ...
            // Add checked items to pantry (assuming you have a pantry table)
