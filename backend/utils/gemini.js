...
export const generateRecipe = async ({ ingredients, dietaryRestrictions = [], cuisineType = 'any', servings = 4, cookingTime = 'medium'}) => {
    const dietaryInfo = dietaryRestrictions.length > 0 ? `The recipe should be suitable for the following dietary restrictions: ${dietaryRestrictions.join(', ')}.` : 'No dietary restrictions';
    ...
    try {
        ...
        // Remove markdowm code blocks if present
...
export const generatePantrySuggestions = async (pantryItems, epiringItems = []) => {
     ...
     const prompt = `Based on these available ingredients: ${ingredients}${expiringText}
                    Suggest 3 creative recipe ideas that use these ingredients. Return ONLY a JSON array of strings (no mardown):
...
export const generateCookingTips = async (recipe) => {
    const prompt = `for this recipe: "${recipe.name}"
                    ...
                    Provide 3-5 helpful cooking tips to make recipe better. Return ONLY a JSON array of strings (no markdown)
